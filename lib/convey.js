	...
	async.forEachSeries(Object.keys(this.config), function (database, next) {
		...
		db.get('convey-version', function (e, conveyVersions) {
			...
			if (e && (e.reason === 'missing' || e.reason === 'deleted')) {
				...
				convey.emit('database:untouched', {
					database: database
				});
				conveyVersions = {
					_id: 'convey-version',
			...
			async.forEachSeries(Object.keys(resources), function (name, next) {
				...
				async.series({
					...
					update: function (next) {
						...
						convey.update(db, designDoc.convey, next);
					}
				}, function (e, results) {
					if (e) return next(e);
...
Convey.prototype.configure = function (config) {
	...
	try {
		...
		return this.emit('error', new Error('configuration file could not be loaded/parsed. ensure a valid JSON config file. - ' + path.join(process.cwd(), config)));
	}
};
